<?php
namespace Aitasty\captcha;

class Captcha
{

	protected $config = [
        'priKey'         => 'Aitasty.captcha',
        // 验证码加密密钥
    ...
    public function create()
    {
        ...
        if ($this->useArithmetic) {
            ...
            $content = $this->createByCode( $code );
            $token['imtext'] = $code;
            $token['code']   = $code[0] + $code[2];
            $hash = $this->authcode( $token['code'] );
        } else {
            if ($this->useZh) {
                // 中文验证码
            ...
            $content = $this->createByCode( $code );
            $token['imtext'] = $code;
            $token['code'] = implode('', $code);
            $hash = $this->authcode( $token['code'] );
        }
        $token['time'] = time();
        $token['hash'] = $hash;
        $this->key_shuffle( $token );
        $result = [
                'token'          => $this->encrypt( json_encode($token,JSON_UNESCAPED_UNICODE) ),
                'content'        => $content,
     ...
     * @param string $token (hash,code,time)[哈密,原始验证码,生成时间]
    ...
    public function check( $code, $token = '')
    {
        // 验证码不能为空
        if (empty($code)) {
            $this->error = ['code'=>-1,'msg'=>'验证码不能为空'];
            return false;
        }
        // token 不能为空
        if (empty( $token ) ){
            $this->error = ['code'=>-2,'msg'=>'token不能为空'];
            return false;
        }
        $token   = $this->decrypt( $token );
        $token   = !is_null(json_decode($token)) ? json_decode( $token,true ) : [];
        $oriCode = isset( $token['code'] ) ? $token['code'] : '';
        //$hash    = isset( $token['hash'] ) ? $token['hash'] : '';
        $verify_time = isset( $token['time'] ) ? (int)$token['time'] : 0;
        // 已过期
        if (time() - $verify_time > $this->expire) {
            $this->error = ['code'=>-3,'msg'=>'已过期'];
            return false;
        }
        // 验证码
        if( !is_numeric($code) && $this->capsLock==false ){
                $code   = strtoupper($code);
                $oriCode= strtoupper($oriCode);
        }
        if ($oriCode==$code) {
            return true;
        }
        $this->error = ['code'=>-4,'msg'=>'验证码错误'];
        return false;
    }
    /**
    * 根据token显示验证码
    */
    ...
    {
        if (empty( $token ) ){
            $this->error = ['code'=>-1,'msg'=>'token不能为空'];
            return false;
        } 
        $token = $this->decrypt( $token );
        $token = !is_null(json_decode($token)) ? json_decode( $token,true ) : [];
        $imtext = isset( $token['imtext'] ) ? $token['imtext'] : '';
        if( empty( $imtext ) ){
            $this->error = ['code'=>-2,'msg'=>'token错误'];
            return false;  
        }
        return $this->createByCode( $imtext );
    }
    ...
    */
    protected function createByCode( $code )
    {
        if( empty($code) || !is_array( $code ) ){
                return false;
        }
        // 图片宽(px)
        $this->imageW = $this->length * $this->fontSize * 1.5 + $this->length * $this->fontSize / 2;
        // 图片高(px)
        ...
        if ($this->useArithmetic) {
            $fontttf = $ttfPath . '6.ttf';
            foreach ($code as $key => $char) {
                $codeNX += $this->fontSize * 1.5;
                imagettftext($this->im, $this->fontSize, 0, $codeNX, $this->fontSize * 1.6, $this->color, $fontttf, $char);
            }
        }else{
            // 验证码使用随机字体
            ...
            if ($this->useZh) {
                // 中文验证码
                foreach( $code as $key => $char ){
                    imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(10, 20), $this->color, $fontttf, $char);
                }
            } else {
                foreach( $code as $key => $char ){
                    $codeNX   += mt_rand($this->fontSize * 1.2, $this->fontSize * 1.6);
                    imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.6, $this->color, $fontttf, $char );
                }
            }
        }
        ob_start();
    ...
    {
        if( !is_numeric($str) && $this->capsLock==false ){
                $str = strtoupper($str);
        }
        $key = substr(md5($this->priKey), 5, 8);
        $str = substr(md5($str), 8, 10);
        return md5($key . $str);
    }

    private function keyNum( $key )
    {
        $num = 0;
    ...
    private function decode64($data) { 
    ...
    private function encrypt( $code ){
        if(empty( $code ))
                return $code;
        $code = (string)$code;
        $keyNum = $this->keyNum( $this->priKey );
        for( $i=0;$i<strlen( $code );$i++){
            $code[$i] = chr( ord($code[$i])+$i+$keyNum );
        }
        $encode = str_replace(array('==','='),'',$this->encode64( ~$code ));
        return $encode;
    }
    private function decrypt( $encode ){
        if(empty( $encode))
                return $encode;
        $decode = ~$this->decode64( $encode );
        $keyNum = $this->keyNum( $this->priKey );
        for( $i=0;$i<strlen( $decode );$i++){
            $decode[$i] = chr( ord($decode[$i])-$i-$keyNum );
        }
        return $decode;
    }
    private function key_shuffle( &$arr )
    {
        if( !empty( $arr )){
            $keys = array_keys( $arr );
            shuffle( $keys );
